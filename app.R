...
# setwd("C:/Users/beltrans/Box/Beltran-Sanchez/Book Mortality in LAC/Chapter_causes_of_death/Comparison ASDR/McKeown")
  ...
  output$plot_multi_country <- renderPlot({
    ...
    plot_data <- cd.age.std.long() %>%
      filter(Country.lab %in% ctries & 
               (age.std.rate != 0 & !is.na(age.std.rate)) & cause %in% c("Waterborne", "Airborne", "Vectorborne") )
    ...
    ggplot(plot_data, 
      ...
      theme(
        ...
        legend.text=element_text(size=15),
      	legend.title=element_text(size=10), legend.justification=c(1,0),      	
